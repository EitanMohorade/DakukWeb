            ...
            <x-dropdown align="top" contentClasses="flex flex-col" width='20'>
                ...
                <x-slot name="content">
                    @foreach ($paginationValues as $value)
                        <button wire:click="setPagination({{ $value }})" class="w-full {{ $categoriesPerPage == $value ? 'bg-green-100' : 'bg-white hover:bg-gray-50'}}">{{ $value }}</button>
                    @endforeach
                </x-slot>
            </x-dropdown>
    ...
    <div class="block w-full overflow-x-auto rounded-t-xl bg-sky-100 shadow-lg">
        <x-table.table>
            <x-slot name="head">
                ...
                <x-table.header :align="'left'" sortable wire:click="sortBy('category_id')" :direction="$sortField == 'category_id' ? $sortDirection : null" value="Padre">Padre</x-table.header>
                        ...
                        <tr class="odd:bg-white even:bg-gray-100" wire:loading.class="opacity-75">
                            ...
                            <x-table.cell>
                                <span
                                    ...
                                    <span class="w-2 h-2 mr-1 bg-{{ $category->status_color }}-500 rounded-full"></span>
